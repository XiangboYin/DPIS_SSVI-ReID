...
def cpu_sk_ir_trainloader(args, main_net, trainloader, tIndex, n_class, print_freq=50):
    ...
    feats = []
    with torch.no_grad():
        for batch_idx, (_, input_rgb, input_ir, label_rgb, label_ir, _, _) in enumerate(trainloader):
            t = time.time()
            input_ir = input_ir.cuda()
            feat, p, _ = main_net(input_ir, input_ir, modal=2, train_set=False)
    ...
    # optimize label using Sinkhorn-Knopp algorithm
