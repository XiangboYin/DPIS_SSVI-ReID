...
import collections
from ChannelAug import ChannelAdap, ChannelAdapGray, ChannelRandomErasing, RandomErasing
...
def pre_process_regdb(args, data_dir):
    ...
    # np.save(os.path.join(data_dir, 'train_regdb_ir_label.npy'), train_thermal_label)
...
class SYSUData(data.Dataset):
    def __init__(self, args, data_dir, transform_train_rgb=None, transform_train_ir=None, colorIndex=None,
                 thermalIndex=None, trainset=True):
        # Load training images and labels
        self.train_color_image, self.train_color_label, self.train_thermal_image, self.train_thermal_label = pre_process_sysu(
            args, data_dir)
        # self.train_thermal_label=np.random.randint(0, 395, len(self.train_thermal_label))
        if args.setting == "unsupervised":
            ...
            self.train_color_label = np.load('clustercontrast/labelfiles/PLG_SYSU_RGB.npy')
        ...
        self.transform_color = transforms.Compose([
            ...
            transforms.Pad(10),
            transforms.RandomCrop((self.args.img_h, self.args.img_w)),
    ...
    def __getitem__(self, index):
        ...
        if self.trainset:
            img0 = self.transform_train_rgb(img1)
            img1 = self.transform_color(img1)
            img2 = self.transform_train_ir(img2)
            return img0, img1, img2, label1, label2, self.cIndex[index], self.tIndex[index]

        img1 = self.transform_train_rgb(img1)
        img2 = self.transform_train_ir(img2)
        return img1, img2, label1, label2, self.cIndex[index], self.tIndex[index]

    def __len__(self):
        return len(self.train_color_label)


class SYSUData_E(data.Dataset):
    def __init__(self, args, data_dir, transform_train_rgb=None, transform_train_ir=None, colorIndex=None,
                 thermalIndex=None):
        # Load training images and labels
        self.train_color_image, self.train_color_label, self.train_thermal_image, self.train_thermal_label = pre_process_sysu(
...
class RegDBData(data.Dataset):
    def __init__(self, args, data_dir, transform_train_rgb=None, transform_train_ir=None, colorIndex=None,
                 thermalIndex=None, trainset=True):
        # Load training images and labels
        self.train_color_image, self.train_color_label, self.train_thermal_image, self.train_thermal_label = pre_process_regdb(
            args, data_dir)

        if args.setting == "unsupervised":
            self.train_color_image = np.load(os.path.join(data_dir, args.train_visible_image_path))
            self.train_color_label = np.load('clustercontrast/labelfiles/PLG_RegDB_RGB.npy')
